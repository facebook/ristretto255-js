...
console.log("Testing encodings of small multiples...");
for (i = 0; i < 16; i++) {
    lowlevel.scalarbase(p, lowlevel.gf([i]))
    var res = byteArrayToHex(ristretto.ristretto255_tobytes(p));
    if (!(res === encodings_of_small_multiples[i])) {
        console.log("FAIL: " + i);
        console.log("res: " + res);
        console.log("exp: " + encodings_of_small_multiples[i]);
        break;
    }
}
console.log("... OK");
...
console.log("Testing bad encodings...");
for (i = 0; i < bad_encodings.length; i++) {
    var res = ristretto.ristretto255_frombytes(p, hexToByteArray(bad_encodings[i]));
    if (!(res === -1)) {
        console.log("FAIL: " + i + " for " + bad_encodings[i]);
        break;
    }
}
console.log("... OK");
...
console.log("Testing good encodings...");
for (i = 0; i < encodings_of_small_multiples.length; i++) {
    var res = ristretto.ristretto255_frombytes(p, hexToByteArray(encodings_of_small_multiples[i]));
    if (res === -1) {
        console.log("FAIL: " + i + " for " + encodings_of_small_multiples[i]);
        break;
    }
}
console.log("... OK");
...
console.log("Testing hash-to-point...");
for (i = 0; i < 7; i++) {
    var h = (crypto.createHash('sha512').update(labels[i]).digest());
    var res = byteArrayToHex(ristretto.ristretto255_tobytes(ristretto.ristretto255_from_hash(h)));
    if (!(res === encoded_hash_to_points[i])) {
        console.log("FAIL: " + i);
        console.log("res: " + res);
        console.log("exp: " + encoded_hash_to_points[i]);
        break;
    }
}
console.log("... OK");
...
var HmHmsalt = ["spdNB+lRije70KwqbDI8+9hBpDStqdU30n4sEPByRzi2ZFUs2xqLom1h95AznvrD9u3vzrD+QDKcX7QPsHd5zQ=="]; // H(pw, H(pw)^salt)

console.log("Testing PAKE...");
for (i = 0; i < pw.length; i++) {
    ...
    /* h1 = H(pw) */
    var h1 = ristretto.ristretto255_from_hash(crypto.createHash('sha512').update(pwd).digest());

    /* h2 = H(pw)^r */
    var h2 = [lowlevel.gf(), lowlevel.gf(), lowlevel.gf(), lowlevel.gf()];;
    lowlevel.scalarmult(h2, h1, r);
    assert.equal(new Buffer(ristretto.ristretto255_tobytes(h2)).toString('base64'), randomizedPassword[i]);
    console.log(".. H(pw)^r ... OK");

    /* h3 = (H(pw)^r)^salt */
    var h3 = [lowlevel.gf(), lowlevel.gf(), lowlevel.gf(), lowlevel.gf()];
    lowlevel.scalarmult(h3, h2, s);
    assert.equal(new Buffer(ristretto.ristretto255_tobytes(h3)).toString('base64'), randomizedSaltedPassword[i]);
    console.log(".. H(pw)^r^salt ... OK");

    /* h6 = H(pw)^salt */
    h1 = ristretto.ristretto255_from_hash(crypto.createHash('sha512').update(pwd).digest());
    var h6 = [lowlevel.gf(), lowlevel.gf(), lowlevel.gf(), lowlevel.gf()];;
    lowlevel.scalarmult(h6, h1, s);
    if (new Buffer(ristretto.ristretto255_tobytes(h6)).toString('base64') != Hmsalt[i]) {
        console.log(".. H(pw)^salt ... FAILED");
        console.log("Expected: " + byteArrayToHex(ristretto.ristretto255_tobytes(h6)));
        console.log("Result  : " + byteArrayToHex([...Hmsalt_exp]));
        break;
    }
    console.log(".. H(pw)^salt ... OK");
    ...
    assert.equal(one.toString(), ristretto.scalarmodL1.toString());
    ristretto.MmodL(one, r, inv_r_exp);
    assert.equal(byteArrayToHex(inv_r_exp), byteArrayToHex(inv_r));
    console.log(".. 1/r ... OK");

    /* h4 = h3^inv_r */
    var h4 = [lowlevel.gf(), lowlevel.gf(), lowlevel.gf(), lowlevel.gf()];
    lowlevel.scalarmult(h4, h3, inv_r);
    assert.equal(byteArrayToHex(ristretto.ristretto255_tobytes(h6)), byteArrayToHex(ristretto.ristretto255_tobytes(h4)));
    console.log(".. H(pw)^r^salt^(1/r) ... OK");

    var h5 = crypto.createHash('sha512').update(pwd).update(ristretto.ristretto255_tobytes(h4)).digest();
    assert.equal(h5.toString('base64'), HmHmsalt[i]);
    console.log(".. H(pw, H(pw)^salt) ... OK");
}
